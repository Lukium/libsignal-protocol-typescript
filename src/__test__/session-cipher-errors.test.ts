...
const createRecordWithSession = (
    ...
    if (chain) {
        const pub =
            session.currentRatchet.ephemeralKeyPair?.pubKey ?? createBuffer(8, 32)
...
describe('SessionCipher error handling', () => {
    ...
    test('encryptJob rejects when identity key missing', async () => {
        ...
        await expect((cipher as any).encryptJob(createBuffer(1, 4))).rejects.toThrow('cannot encrypt without identity key')
    })
    ...
    test('fillMessageKeys guards against large future counters', async () => {
        ...
        await expect((cipher as any).fillMessageKeys(chain, 2500)).rejects.toThrow('Over 2000 messages into the future!')
    })
